...
## v1.0.17（2026-07-21）
- 🎨 **整个应用背景改为渐变（液态玻璃风格）**：所有主题的 `windowBackground` 改用对应色调渐变 drawable（翡翠绿/蓝/紫/红/曜金黑/液态玻璃），Activity 与 Fragment 根布局改为透出窗口渐变，全局统一渐变背景
- 🔘 **企业查询「源设置」重做为复选框列表**：内置 7 个公开数据源（爱企查、企查查、天眼查、国家公示系统、启信宝、水滴信用、企查猫），**默认全选**，可逐个取消勾选
- ➕ **新增「自定义 API 源」**：在源设置顶部可填写名称 + 搜索网址（用 `%s` 代替关键词）+ 可选 API Key，自动加入聚合队列，满足自带接口的需求
- 💠 **蓝渐变主题改为文强网官网白科技风配色**：原「深海蓝」主题背景由深蓝重渐变改为与官网亮色同款的浅白底蓝紫渐变（`#eaf0ff→#dbe4ff→#eadcff`），主色科技蓝 `#3a66ff`，更清爽通透

## v1.0.16（2026-07-21）
- 🌐 **修复关于页官网链接**：「官网」按钮跳转地址由 `https://johnny520.github.io/`（用户站根目录）修正为正确的 `https://johnny520.github.io/Johnny/`，补齐缺失的 `Johnny` 路径段

## v1.0.15（2026-07-21）
- 🕷️ **企业查询改为「多源免费爬虫聚合」**：彻底移除鲸海数据（kqdaas）API 与内置密钥方案，改用应用内隐藏 WebView 依次爬取爱企查、企查查、天眼查、国家公示系统四家公开工商信息网站，JS 抽取列表后原生卡片聚合并去重，每张卡片标注数据来源；无需任何 API Key
- 🔀 **多源顺序回退 + 进度提示**：任一源被反爬/超时则优雅跳过，其余源继续；状态栏实时显示「正在爬取 / 找到 N 条 / 已跳过」
- ⚙️ **数据源可开关**：「源设置」中可勾选要爬取的数据源（默认全开），某源长期不可用可单独关闭
- 📖 **新增「说明」弹窗**：告知免费、无 Key、数据仅供参考等
- 🧹 清理：删除 kqdaas 残留（build.gradle 字段、API 设置对话框、相关字符串）及更早 gsxt 版遗留布局

## v1.0.13（2026-07-21）
- 🔍 **安装包提取：搜索框 + 常驻加载弹窗**：进入即显示「正在加载应用列表…」小弹窗，列表加载完成自动消失，告别“点进去半天不出 APP”；顶部新增搜索框，按名称/包名实时过滤，无需逐个滑动翻找
- 🎯 **快速到顶/到底按钮合并为方向键式小按钮**：原两个独立悬浮按钮合并为一个小巧按钮（⇕），点击弹出「快速到顶 / 快速到底」，更省空间
- 🧠 **修复内存加速“用不了”**：补齐 `KILL_BACKGROUND_PROCESSES` 权限（`am.killBackgroundProcesses` 此前因缺权限被系统静默拒绝，导致清理 0 个）
- ℹ️ **关于页新增个人主页与官网链接**：可一键跳转 GitHub 个人主页与官网
- 🔢 **版本号对齐 GitHub Release**：`versionName` 升至 1.0.13（此前部分安装包仍显示旧版本号）
