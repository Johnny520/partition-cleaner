...
public class Cleaner {
    ...
    public static String clean(RootShell root, List<JunkItem> items) {
        return cleanInternal(root, items, true);
    }

    /** 无视勾选状态，清理列表中所有项（用于“只清建议清理”）。 */
    public static String cleanForce(RootShell root, List<JunkItem> items) {
        return cleanInternal(root, items, false);
    }

    private static String cleanInternal(RootShell root, List<JunkItem> items, boolean respectSelection) {
        if (root == null || !root.isAvailable()) {
            return "未获取 Root 权限，无法清理系统分区";
        }
        int ok = 0;
        int fail = 0;
        for (JunkItem it : items) {
            if (respectSelection && !it.selected) continue;
            String r = cleanOne(root, it);
            if (r.contains("OK")) ok++;
            else fail++;
        }
    ...
    private static String cleanOne(RootShell root, JunkItem it) {
        // 系统分区需先以读写方式重新挂载
        String mp = mountPointOf(it.path);
        if (mp != null) {
            root.run("mount -o remount,rw " + mp + " 2>/dev/null");
        }
        String rm;
        ...
        return root.run(rm + " && echo OK || echo FAIL");
    }
